...
- 실행 경로/포트/키 위치를 환경 변수로 분리하면 배포 환경이 바뀌어도 코드 변경 없이 대응할 수 있습니다.
- 하드코딩을 줄이면 “개발 환경에서는 동작하지만 운영에서는 실패” 같은 환경 차이 문제를 줄일 수 있습니다.
- 핵심 실행 변수는 시작 시점에 검증 가능해야 하며, 실패 시 빠르게 원인을 식별할 수 있어야 합니다.
- 검증은 애플리케이션 부트 로그 + 실제 리슨 포트/파일 접근 상태를 함께 확인하는 방식이 가장 안전합니다.

### 5. 관제 스크립트 + 로그 기록으로 장애 추적이 쉬워지는 이유

- 관제의 핵심은 “현재 생존 여부 확인”과 “시간축 데이터 축적”을 동시에 수행하는 것입니다.
- 프로세스/포트/리소스 상태를 주기 기록하면 장애 시점 전후의 변화를 근거 기반으로 분석할 수 있습니다.
- 알림만 있고 기록이 없으면 원인 분석이 감에 의존하게 되고, 재발 방지 품질이 떨어집니다.
- 운영 관제는 보통 “수집 -> 판단(임계치) -> 기록 -> 분석/개선” 루프로 지속 개선됩니다.
...
- 프로세스 확인: `pgrep -f "$APP_PATTERN"`
  ...
  - PID를 바로 얻어 로그 포맷(`PID:...`)에 재사용 가능
- 포트 확인: `ss -tuln | awk ...`
  - 이유: `ss`는 최신 리눅스에서 `netstat` 대비 기본 탑재 가능성이 높고 빠름
  - `tcp + LISTEN + :15034` 조건을 정확히 확인해 Health Check 실패 여부를 결정
